...
// if an entry exists in the map. If no such entry exists, a node with the
// same label as 'node_id' is created in the transformed graph and the
// identifier of this new node is returned.
NodeId FindOrCopyNode(NodeId node_id, Transformation* transform) {
  NodeId new_node;
  std::map<NodeId, NodeId>& node_map = transform->node_map;
  auto map_it = node_map.find(node_id);
  if (map_it == node_map.end()) {
    new_node = transform->output->FindOrAddNode(
        transform->input.GetNodeLabel(node_id));
...
// function uses a helper function FindOrCopyNode but not a helper function
// FindOrCopyEdge because edges in the input graph are traversed in order of
// their source nodes. A FindOrCopyEdge function would not know that the source
// node of an edge in a sequence of calls is the same and would perform
...
std::unique_ptr<LabeledGraph> DeleteNodes(const LabeledGraph& graph,
  ...
  for (NodeIterator node_it = graph.NodeSetBegin(); node_it != end_it;
    ...
    NodeId new_src = FindOrCopyNode(src, &transform);
